...
export async function getStoreDirectory(ghc: GhcInfo, cabalVersion: string) {
  ...
  const compilerId = `ghc-${ghc.info["Project version"]}`;
  const compilerAbiTag = ghc.info["Project Unit Id"];
